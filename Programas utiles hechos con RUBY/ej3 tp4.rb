...
def calcular(fecha)
    t=Time.now
    time=t.to_s
    aanow=time[0..3].to_i ; mmnow=time[5..6].to_i ; ddnow=time[8..9].to_i ; ddnowactual=time[8..9].to_i
    dd=fecha[0..1].to_i ; mm=fecha[3..4].to_i ; aa=fecha[6..9].to_i
    conta=0 ; contm=0 ; contd=0 ; igualdad=0
    ...
    if(igualdad==0)
      begin
        ...
        case
        when((ddnow==dd) and (contm==12))
          conta=conta+1
          contm=0
        when((ddnow==ddnowactual))
          contm=contm+1
          contd=0
        end
        contd=contd+1
      end until((aanow==aa) and (mmnow==mm) and (ddnow==dd)) 
      if((aanow==aa) and (mmnow==mm) and (ddnow==dd) and (conta==0))
        contm=0
        conta=conta+1
      end
      puts"La edad de la persona de la fecha ingresada hasta la fecha actual:"
      puts"-#{conta} años, #{contm} meses y #{contd} dias."
    else
